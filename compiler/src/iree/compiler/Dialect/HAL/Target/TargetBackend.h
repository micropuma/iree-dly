...
public:
  ...
  //
  // ===================== 对于sample HAL ir code的解读：=====================
  // @main_io 是一个接口，定义了程序与硬件之间的输入输出映射，具体指示了输入输出参数（如 @arg0 和 @arg1）如何与硬件资源绑定。
  // @main 是程序的主函数，它通过 @main_io 接口与硬件交互，执行具体的计算任务。换句话说，@main 的输入参数和输出结果通过 @main_io 接口来传递。
  // 这种绑定关系意味着 @main 函数在执行时，输入参数会被从硬件资源中读取，输出结果会被写入到硬件资源中。
